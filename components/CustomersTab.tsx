...
function InlineCurrencyInput({ initialValue, onSave }: { initialValue: number | undefined, onSave: (val: number | undefined) => void }) {
  const [localValue, setLocalValue] = useState<string>(initialValue ? initialValue.toString() : '');
  const [isFocused, setIsFocused] = useState(false);

  useEffect(() => {
    if (!isFocused) {
      setLocalValue(initialValue ? initialValue.toString() : '');
    }
  }, [initialValue, isFocused]);

  const displayValue = localValue ? new Intl.NumberFormat('vi-VN').format(Number(localValue)) : '';
  ...
  return (
    <input
      ...
      onBlur={handleBlur}
      className="border border-gray-200 rounded px-2 py-1 text-xs outline-none focus:border-blue-500 bg-white w-full text-right"
      placeholder="Nhập..."
    />
  );
}
...
export default function CustomersTab({ currentUser }: CustomersTabProps) {
  ...
  const [customerDirectorySearchTerm, setCustomerDirectorySearchTerm] = useState('');
  ...
  const customerSummaries = customers
    ...
    const contractValue = periodInitialDebt + totalPurchase;
    const remainingDebt = customer.actualValue !== undefined && customer.actualValue !== null 
      ? customer.actualValue - totalPaid 
  ...
  const totalSummaryContract = customerSummaries.reduce((sum, c) => sum + c.contractValue, 0);
  const totalSummaryActual = customerSummaries.reduce((sum, c) => sum + (c.actualValue !== undefined && c.actualValue !== null ? c.actualValue : 0), 0);
  ...
  const renderSummaryTable = () => (
    <div className="space-y-4 w-full px-2" style={{ minWidth: '1200px' }}>
      ...
      <div className="overflow-x-auto overflow-y-auto max-h-[70vh]">
        <table className="w-full min-w-[800px] text-[11px] md:text-sm text-left relative">
          <thead className="bg-gray-50 text-gray-600 font-medium border-b border-gray-100 sticky top-0 z-20 whitespace-nowrap">
            <tr>
              ...
              {showContractAndPaid && <th className="px-2 md:px-4 py-3 text-right w-[120px]">Giá Trị TT</th>}
          ...
          <tbody className="divide-y divide-gray-100">
            <tr className="bg-blue-50/80 font-bold text-blue-900 border-b-2 border-blue-200">
              ...
              {showContractAndPaid && <td className="px-2 md:px-4 py-3 text-right">{formatCurrency(totalSummaryContract)}</td>}
              {showContractAndPaid && <td className="px-2 md:px-4 py-3 text-right text-blue-700">{formatCurrency(totalSummaryActual)}</td>}
            ...
            {customerSummaries.map((c, index) => {
              ...
              const displayActualValue = c.actualValue ? new Intl.NumberFormat('vi-VN').format(c.actualValue) : '';
              return (
              <tr 
                key={c.id} 
              ...
              >
                ...
                {showContractAndPaid && <td className="px-2 md:px-4 py-3 text-right text-gray-600">{formatCurrency(c.contractValue)}</td>}
                {showContractAndPaid && <td className="px-2 md:px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                  <InlineCurrencyInput
                    initialValue={c.actualValue}
                    onSave={async (actualValue) => {
                      await apiUpdateCustomer({ ...c, actualValue });
                    }}
                  />
                </td>}
            ...
            {customerSummaries.length === 0 && (
              <tr>
                <td colSpan={showContractAndPaid ? 13 : 8} className="px-4 py-8 text-center text-gray-500">
  ...
  const renderCustomersDirectory = () => {
    const filteredDirectoryCustomers = customers.filter(c => 
      c.name.toLowerCase().includes(customerDirectorySearchTerm.toLowerCase()) ||
      c.phone.toLowerCase().includes(customerDirectorySearchTerm.toLowerCase()) ||
      (c.address && c.address.toLowerCase().includes(customerDirectorySearchTerm.toLowerCase()))
    );
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row justify-between gap-4">
        <div className="relative flex-1">
          <input
            type="text"
            placeholder="Tìm kiếm khách hàng (Tên, Điện thoại, Địa chỉ)..."
            value={customerDirectorySearchTerm}
            onChange={(e) => setCustomerDirectorySearchTerm(e.target.value)}
            className="w-full border border-gray-300 rounded-xl pl-10 pt-2 pb-2 outline-none focus:border-blue-500 bg-white"
          />
          <IconSearch className="w-5 h-5 text-gray-400 absolute left-3 top-2.5" />
        </div>
        <button 
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[600px] text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredDirectoryCustomers.map((c, index) => (
