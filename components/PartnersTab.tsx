...
export default function PartnersTab({ currentUser }: PartnersTabProps) {
  ...
  const [partnerDirectorySearchTerm, setPartnerDirectorySearchTerm] = useState('');
  ...
  const renderPartnersDirectory = () => {
    const filteredDirectoryPartners = partners.filter(p => 
      p.name.toLowerCase().includes(partnerDirectorySearchTerm.toLowerCase()) ||
      p.phone.toLowerCase().includes(partnerDirectorySearchTerm.toLowerCase()) ||
      (p.address && p.address.toLowerCase().includes(partnerDirectorySearchTerm.toLowerCase()))
    );
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row justify-between gap-4">
        <div className="relative flex-1">
          <input
            type="text"
            placeholder="Tìm kiếm đối tác (Tên, Điện thoại, Địa chỉ)..."
            value={partnerDirectorySearchTerm}
            onChange={(e) => setPartnerDirectorySearchTerm(e.target.value)}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredDirectoryPartners.map((p, index) => (
