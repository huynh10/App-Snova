...
export default function FinanceTab({ currentUser }: FinanceTabProps) {
  ...
  const reportData = filteredCustomers.map(customer => {
    ...
    const giaTriTT = customer.actualValue;
    ...
    const effectiveValue = (giaTriTT !== undefined && giaTriTT !== null) ? giaTriTT : giaTriHD;
    const loiNhuan = effectiveValue - tongChi;

    return {
      ...customer,
      giaTriHD: effectiveValue,
      tongThu,
      tongChi,
      loiNhuan
    };
    ...
    return (
      <div className="space-y-6">
              ...
              <thead className="bg-gray-50 text-gray-600 font-medium border-b border-gray-100 sticky top-0 z-20">
                <tr>
                  ...
                  {showFinancialColumns && (
                    <>
                      <th className="px-2 md:px-4 py-3 text-right">Giá trị HĐ (TT)</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {reportData.length > 0 && (
                  <tr className="bg-blue-50/80 font-bold border-b-2 border-blue-200">
                    ...
                    {showFinancialColumns && (
                      <>
                        <td className="px-2 md:px-4 py-3 text-right text-gray-900">{formatCurrency(reportData.reduce((sum, r) => sum + r.giaTriHD, 0))}</td>
                        ...
                        <td className="px-2 md:px-4 py-3 text-right text-blue-600">{formatCurrency(reportData.reduce((sum, r) => sum + r.loiNhuan, 0))}</td>
                      </>
                    )}
                  </tr>
                )}
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-lg font-bold text-gray-800 mb-4">Biểu đồ Tài chính Công trình</h3>
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Bar dataKey="giaTriHD" name="Giá trị HĐ (TT)" fill="#3b82f6" radius={[4, 4, 0, 0]} maxBarSize={50} />
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6 relative pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-full p-6 shadow-lg flex flex-col items-center justify-center text-white transform transition-transform hover:scale-[1.02]">
          <span className="text-blue-100 text-sm md:text-base font-medium uppercase tracking-wider mb-1">Tổng giá trị HĐ (hiệu chỉnh)</span>
...
function AddChiModal({ onClose, currentUser, customers, partners, products, financeCategories, initialData }: any) {
  ...
  const [executor, setExecutor] = useState(initialData?.executor || currentUser.name);
  const [note, setNote] = useState(initialData?.note || '');
  const [isPaidDirectly, setIsPaidDirectly] = useState(() => {
    if (initialData?.purchaseAmount > 0 && initialData?.paidAmount === initialData?.purchaseAmount) return true;
    return false;
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isProductCategory) {
      ...
      if (isPaidDirectly) {
        ...
        finalPaidAmount = (wasProductCategory && initialData?.paidAmount !== initialData?.purchaseAmount) ? (initialData?.paidAmount || 0) : 0;
      }
    } else if (isFinanceCategory) {
      if (partnerId) {
          ...
          <div className="p-4 sm:p-6 overflow-y-auto flex-1">
            ...
            <div>
              ...
              <div className="flex gap-4 items-center">
                ...
                {products.some((p: any) => p.id === productId) && partnerId && (
