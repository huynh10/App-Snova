...
function ImportModal({ onClose, currentUser, items, partners }: any) {
  ...
  const [isPaidDirectly, setIsPaidDirectly] = useState(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await apiAddPartnerTransaction({
      ...
      quantity: Number(quantity),
      purchaseAmount: totalPrice,
      paidAmount: isPaidDirectly ? totalPrice : 0,
...
function ExportModal({ onClose, currentUser, items, customers, inventoryStock }: any) {
  ...
  const [exportItems, setExportItems] = useState([
    { id: Date.now().toString(), itemId: '', quantity: '', unitPrice: '' }
  ]);

  const addExportItemRow = () => {
    setExportItems([...exportItems, { id: Date.now().toString(), itemId: '', quantity: '', unitPrice: '' }]);
  };

  const removeExportItemRow = (id: string) => {
    setExportItems(exportItems.filter(item => item.id !== id));
  };

  const updateExportItem = (id: string, field: string, value: string) => {
    setExportItems(exportItems.map((item: any) => {
      if (item.id === id) {
        const newItem = { ...item, [field]: value };
        if (field === 'itemId') {
          const selectedStockItem = inventoryStock.find((i: any) => i.id === value);
          if (selectedStockItem && !newItem.unitPrice) {
            newItem.unitPrice = selectedStockItem.unitPrice.toString();
          }
        }
        return newItem;
      }
  ...
  const calculateTotalForRow = (item: any) => {
    return (Number(item.quantity) || 0) * (Number(item.unitPrice) || 0);
  };

  const calculateTotalAll = () => {
    return exportItems.reduce((acc: number, item: any) => acc + calculateTotalForRow(item), 0);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validItems = exportItems.filter((item: any) => item.itemId && item.quantity);
    if (validItems.length === 0) {
      alert("Vui lòng nhập ít nhất 1 mặt hàng cần xuất");
      return;
    }

    for (const item of validItems) {
      const selectedStockItem = inventoryStock.find((i: any) => i.id === item.itemId);
      ...
      if (exportQty > maxQuantity) {
        ...
        alert(`Số lượng xuất (${exportQty}) của mặt hàng ${selectedItem?.name} không được lớn hơn số lượng tồn kho (${maxQuantity})`);
        return;
      }
    }
    
    const customerTxId = Date.now().toString();
    const itemNames = validItems.map((vi: any) => items.find((i: any) => i.id === vi.itemId)?.name).filter(Boolean);
    const content = `Xuất kho: ${itemNames.join(', ')}`;
    const totalAmount = calculateTotalAll();
    const totalQuantity = validItems.reduce((sum: number, item: any) => sum + (Number(item.quantity) || 0), 0);
    ...
    // 2. Create multiple InventoryTransactions
    let idx = 1;
    for (const item of validItems) {
      const inventoryTxId = (Date.now() + idx).toString();
      idx++;
      await apiAddInventoryTransaction({
        id: inventoryTxId,
        companyId: currentUser.companyId,
        ...
        totalPrice: calculateTotalForRow(item),
        customerId,
        note,
        executor: executor,
        createdAt: new Date().toISOString(),
          ...
          <form id="export-form" onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 space-y-4">
              <div className="flex justify-between items-center">
                <h4 className="font-medium text-gray-900">Danh sách mặt hàng xuất</h4>
              </div>
                  ...
                  return (
                    <div key={item.id} className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm relative">
                      {exportItems.length > 1 && (
                        <button 
                          type="button"
                          onClick={() => removeExportItemRow(item.id)}
                          className="absolute -top-2 -right-2 bg-red-100 text-red-600 rounded-full p-1 border border-red-200 hover:bg-red-200 transition-colors"
                        >
                          <IconTrash className="w-4 h-4" />
                        </button>
                      )}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
                        <div className="md:col-span-5">
                          ...
                          <div className="relative">
                            <SearchableSelect 
                              options={items}
                              value={item.itemId}
                              onChange={(val: string) => updateExportItem(item.id, 'itemId', val)}
                              placeholder="-- Chọn hàng hóa --"
                            />
                            {item.itemId && (
                              <div className="absolute right-0 top-[-20px] text-xs font-medium text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-100">
                                Tồn: {maxQuantity} {items.find((i: any) => i.id === item.itemId)?.unit}
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="md:col-span-2">
                          ...
                          <input 
                            ...
                            step="0.01" 
                            value={item.quantity} 
                            onChange={e => updateExportItem(item.id, 'quantity', e.target.value)} 
                        ...
                        <div className="md:col-span-3">
                          <CurrencyInput 
                            ...
                            onChange={(val: string) => updateExportItem(item.id, 'unitPrice', val)} 
                            required={true} 
                          />
                        </div>
                        ...
                        <div className="md:col-span-2">
                          ...
                          <div className="w-full border border-gray-100 rounded-lg px-2 py-2 bg-gray-50 text-gray-900 font-medium text-sm text-right overflow-hidden text-ellipsis md:mt-0">
                            {new Intl.NumberFormat('vi-VN').format(calculateTotalForRow(item))}
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <button 
                type="button" 
                onClick={addExportItemRow}
                className="flex items-center gap-1 text-sm font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 px-3 py-2 rounded-lg transition-colors"
              >
                <IconPlus className="w-4 h-4" /> Thêm mặt hàng khác
              </button>
            </div>

            <div className="flex flex-col md:flex-row justify-between items-end border-t border-gray-100 pt-4 gap-4">
              <div className="flex-1 w-full">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Ghi chú</label>
                    <textarea value={note} onChange={e => setNote(e.target.value)} rows={1} className="w-full border border-gray-300 rounded-xl px-4 py-2.5 outline-none focus:border-blue-500"></textarea>
                  </div>
                </div>
              </div>
              <div className="text-right shrink-0">
                <label className="block text-sm font-medium text-gray-700 mb-1">Tổng tiền xuất</label>
                <div className="text-2xl font-bold text-orange-600">
                  {new Intl.NumberFormat('vi-VN').format(calculateTotalAll())} đ
                </div>
              </div>
            </div>
